...
def getExportUrl(url, filename, printv=False, exportPPTX=True):
...
@click.option('--epptx','-e',help ="If true, will export files into PPTX and then PNG files, default = True",default = True,type=bool)
@click.option('--log','-l',help ="If true will print variables in every stage, default = False",default = False,type=bool)

def main(item,epptx,log):
    click.echo(item)
    for it in item:
        print(it.split())
        it = it.split()
        try:
            getExportUrl(it[0],it[1],printv=log, exportPPTX=epptx)
